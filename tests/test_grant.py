...
def test_compile_grant_table_column(target, schema, compiled):
    statement = grant(["ALL (a)"], PgObjectType.TABLE, target, "alice", schema=schema)
    assert str(statement.compile(dialect=postgresql.dialect())) == compiled
...
@pytest.mark.parametrize(
    "target, arg_types, schema, compiled",
    [
...
@pytest.mark.parametrize(
    "grantee, privs, privswgo, type_, target, kw",
    [
